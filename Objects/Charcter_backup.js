...
let characterMovement = { direction: 1, axis: "z" };

let clock = new THREE.Clock();

function character_init(publicObject, glb) {
  ...
  function keydownFunc(e) {
    ...
    // if (key_code === 65) mx += mx * 5 * mixerUpdateDelta;
    // console
    if (key_code === 87) {
    ...
    // if (key_code === 68) mx += mx * 5 * mixerUpdateDelta;
    // if (key_code === 83) mz += mz * 5 * mixerUpdateDelta;
...
function character_anim(publicObject) {
  ...
  if (model !== undefined) {
    if (characterAnimState === "run") {
      const moveZ = 1 * 4 * mixerUpdateDelta;
...
export { character_init, character_anim };
